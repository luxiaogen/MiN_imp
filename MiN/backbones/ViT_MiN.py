...
class PiNoise(nn.Module):
    def __init__(
            ...
            noise_sigma_init=-6.0,
    ):
        super().__init__()

        self.hidden_dim = hidden_dim
        self.use_reparam_noise = use_reparam_noise
        ...
        self.register_buffer("w_down", torch.empty((in_dim, self.hidden_dim)))
        self.register_buffer("w_up", torch.empty((self.hidden_dim, out_dim)))
        self._reset_matrix(self.w_down)
        self._reset_matrix(self.w_up)
    ...
    def update_noise(self):
        ...
        if len(self.mu) == 0:
            ...
            torch.nn.init.constant_(self.sigmma[0].bias, self.noise_sigma_init if self.use_reparam_noise else 0.)
        else:
            self.mu.append(nn.Linear(self.hidden_dim, self.hidden_dim))
            ...
            torch.nn.init.constant_(self.sigmma[-1].bias, self.noise_sigma_init if self.use_reparam_noise else 0.)
    
    def init_weight_noise(self, prototypes):
        if len(self.mu) == 0:
            return

        device = self.mu[-1].weight.device
        weight = torch.ones(len(self.mu), device=device)

        if len(prototypes) == 0:
            weight = torch.zeros(len(self.mu), device=device)
            weight[-1] = 1.0
        else:
            mu_t = prototypes[-1].to(device)
            for i in range(len(prototypes)):
                mu_i = prototypes[i].to(device)
                dot_product = torch.dot(mu_t, mu_i)
                norm_t = torch.norm(mu_t)
                norm_i = torch.norm(mu_i)
                denom = (norm_t * norm_i).clamp_min(1e-12)
                weight[i] = (dot_product / denom).detach().clone()

        self.weight_noise = torch.softmax(weight, dim=-1)
    ...
    def _get_branch_weight(self, idx, device, dtype):
        if self.weight_noise is None:
            weight = torch.zeros(len(self.mu), device=device, dtype=dtype)
            weight[-1] = 1.0
            return weight[idx]

        return self.weight_noise[idx].to(device=device, dtype=dtype)

    def _compute_branch_noise(self, x_down, idx, sample_noise=False, should_sample=False):
        mu = self.mu[idx](x_down)
        raw_sigma = self.sigmma[idx](x_down)

        if not self.use_reparam_noise:
            return mu + raw_sigma

        if not sample_noise or not should_sample:
            return mu
    ...
    def forward(self, hyper_features, sample_noise=False, sample_mode="latest_only", new_forward=False):
        ...
        if new_forward:
            branch_indices = [len(self.mu) - 1]
        else:
            branch_indices = range(len(self.mu))

        for i in branch_indices:
            should_sample = sample_noise and (
                sample_mode == "all" or i == len(self.mu) - 1
            )
            branch_noise = self._compute_branch_noise(
                x_down, i, sample_noise=sample_noise, should_sample=should_sample
            )
            branch_weight = self._get_branch_weight(
                i, branch_noise.device, branch_noise.dtype
            )
            if noise is None:
                noise = branch_noise * branch_weight
            else:
                noise += branch_noise * branch_weight
    ...
    def forward_new(self, hyper_features, sample_noise=False, sample_mode="latest_only"):
        return self.forward(
            hyper_features,
    ...
    def __init__(
        ...
        self.noise_maker = nn.Sequential(*[
            PiNoise(
                ...
                768,
                args['hidden_dim'],
                use_reparam_noise=args.get('use_reparam_noise', False),
                noise_sigma_init=args.get('noise_sigma_init', -6.0),
            ) for i in range(depth)
        ])
