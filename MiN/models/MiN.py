...
class MinNet(object):
    def __init__(self, args, loger):
        ...
        self.log_histograms = args.get("logger_histograms", False)
    ...
    def _log_noise_histograms(self, step):
        if not self.log_histograms:
            return

        for layer_idx, noise_module in enumerate(self._network.backbone.noise_maker):
            if noise_module.weight_noise is not None:
                self.experiment_logger.log_histogram(
                    f"noise/layer_{layer_idx}/weight_noise",
                    noise_module.weight_noise.detach().cpu(),
                    step,
                )
    ...
    def after_train(self, data_manger):
        ...
        self.experiment_logger.log_scalar(
            "eval/task_acc",
            float(eval_res['all_class_accy'] * 100.0),
            self.global_eval_step,
        )
        self.experiment_logger.log_scalar(
            "eval/avg_acc",
            float(np.mean(self.total_acc)),
            self.global_eval_step,
        )
    ...
    def run(self, train_loader):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                inputs, targets = inputs.to(self.device), targets.to(self.device)
                buffered_features = self._network.encode_buffered(
                    inputs,
                    new_forward=False,
                    sample_noise=True,
                    sample_mode=self.noise_sample_mode,
                ...
                if self.cur_task > 0:
                    with torch.no_grad():
                        outputs1 = self._network.forward_from_buffer(
                            buffered_features.detach()
                        )
                        logits1 = outputs1['logits']
                    outputs2 = self._network.forward_normal_fc_from_buffer(
                        buffered_features
                    )
                    ...
                    outputs = self._network.forward_normal_fc_from_buffer(
                        buffered_features
                    )
                    logits = outputs["logits"]
            ...
            self.experiment_logger.log_scalar("train/epoch_loss", avg_loss, self.global_train_step)
            self.experiment_logger.log_scalar("train/epoch_acc", float(train_acc), self.global_train_step)
            self.experiment_logger.log_scalar("train/task_id", self.cur_task, self.global_train_step)
            self._log_noise_histograms(self.global_train_step)
