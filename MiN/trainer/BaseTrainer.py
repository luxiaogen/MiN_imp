...
def _train(args):
    ...
    if not os.path.exists(workdir):
        os.makedirs(workdir)
    args["log_dir"] = logs_name
    args["workdir"] = workdir
    args["tensorboard_log_dir"] = os.path.join(
        logs_name, args.get("tensorboard_subdir", "tensorboard")
    )
    ...
    try:
        ...
        for i in range(datamanger.task_size): # 把后续所有增量 task 依次训练完，并按需要保存 checkpoint；最后再保存一个最终模型
